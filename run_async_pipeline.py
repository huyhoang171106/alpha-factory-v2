...
from alpha_ranker import (
    ...
    apply_family_crowding_penalty,
    estimate_gate_probability,  # pre-sim gate estimator (Fix 3)
)
...
SIMULATOR_WORKERS = max(
    1, int(os.getenv("ASYNC_SIMULATOR_WORKERS", "1"))
)  # WQ API has global concurrency limits
...
ADAPTIVE_GATES = os.getenv("ASYNC_ADAPTIVE_GATES", "1").strip().lower() in (
    "1",
    "true",
    "yes",
)
ADAPTIVE_MIN_PRE_RANK = float(
    os.getenv("ASYNC_ADAPTIVE_MIN_PRE_RANK", "48.0")
)  # raised from 42
ADAPTIVE_MAX_PRE_RANK = float(
...
LOCAL_BT_ENABLED = os.getenv("ASYNC_LOCAL_BT", "1").strip().lower() in (
    "1",
    "true",
    "yes",
)  # enabled by default to pre-filter before WQ sim
LOCAL_BT_MIN_SCORE = float(
    os.getenv("ASYNC_LOCAL_BT_MIN_SCORE", "55.0")
)  # raised from 45
LOCAL_BT_MIN_SHARPE = float(
    os.getenv("ASYNC_LOCAL_BT_MIN_SHARPE", "0.40")
)  # raised from 0.20
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("alpha_async_factory.log")],
)
...
class AsyncAlphaFactory:
    def __init__(
        ...
        self.generator = (
            ...
            else AlphaGenerator(generation_mode=os.getenv("GENERATOR_MODE", "legacy"))
        )
        self.governor = (
            governor
            if governor is not None
            else SubmitGovernor(self.tracker, self.client)
        )
        ...
        self._batch_recent: list[
            str
        ] = []  # rolling in-batch expression cache for similarity check

        for descriptor, expr, quality, novelty in self.tracker.load_qd_archive(
            limit=1800
        ):
    ...
    def adapt_runtime_gates(self, kpi: dict | None = None) -> str:
        ...
        resolved = int(kpi.get("accepted", 0) or 0) + int(
            kpi.get("rejected_after_submit", 0) or 0
        )
    ...
    def _simulation_reward(self, res) -> float:
        ...
        sub_score = (
            0.5 if sub_sharpe <= -0.99 else self._clamp01((sub_sharpe + 0.50) / 1.50)
        )
    ...
    def _passes_local_backtest(self, cand: AlphaCandidate) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.warning(
                "Local backtest disabled after initialization/eval failure: %s", exc
            )
    ...
    async def should_accept_candidate(
        self, cand: AlphaCandidate
        ...
        if (
            ...
            or signature in self.qd_archive.signature_seen
        ):
        ...
        if not should_simulate_candidate(
            cand.expression, min_critic_score=MIN_CRITIC_SCORE
        ):
        ...
        # ── Fix 3: Pre-simulation gate probability estimator ───────────────────
        # Reject candidates that structurally cannot pass the WQ gate thresholds.
        # This prevents wasting WQ Brain quota on doomed alphas.
        gate_est = estimate_gate_probability(cand.expression)
        if not gate_est["passed"]:
            return False, f"gate_est:{gate_est['reason']}", gate_est["gate_score"]
    ...
    async def producer_generator(self):
        ...
        while self.is_running:
            try:
                if (
                    self.gen_queue.qsize() > GEN_QUEUE_SIZE * 0.75
                    or self.sim_queue.qsize() > SIM_QUEUE_SIZE * 0.75
                ):
                    await asyncio.sleep(2)
                    continue
                batch = await loop.run_in_executor(
                    None,
                    lambda: self.generator.generate_batch(
                        n=GEN_BATCH_SIZE, use_rag=ASYNC_USE_RAG
                    ),
                )
                for cand in batch:
            ...
            try:
                ...
                if not accepted:
                    ...
                    if self.stats["filtered"] % 1000 == 0:
                        logger.info(
                            "Ranker high filter rate: %s rejected. Last reason: %s | expr_id=%s",
                            self.stats["filtered"],
                            reason,
                            safe_expr_ref(cand.expression),
                        )
                    continue
                ...
                self.pending_signatures.add(
                    parameter_agnostic_signature(cand.expression)
                )
    ...
    async def worker_simulator(self, worker_id: int):
        ...
        logger.info(
            "⚡ [Simulator-%s] Independent streaming worker started.", worker_id
        )
        loop = asyncio.get_running_loop()
        ...
        while self.is_running:
            ...
            try:
                ...
                try:
                    priority, ts, cand = await asyncio.wait_for(
                        self.sim_queue.get(), timeout=2.0
                    )
                ...
                logger.info(
                    "[Simulator-%s] Processing priority=%s | expr_id=%s",
                    worker_id,
                    priority,
                    safe_expr_ref(cand.expression),
                )
                ...
                res = await asyncio.wait_for(
                    loop.run_in_executor(
                        None, lambda: self.client.simulate(cand.expression, delay=delay)
                    ),
                ...
                if res.error and (
                    "rate_limited" in res.error.lower()
                    or "CONCURRENT_SIMULATION_LIMIT" in res.error.upper()
                ):
                    async with self._pause_lock:
                        if time.time() > self.simulation_pause_until:
                            self.simulation_pause_until = time.time() + 60
                            logger.warning(
                                f"🐢 [Backoff] WQ limit hit by W-{worker_id}. Pausing ALL simulations for 60s."
                            )

            except asyncio.TimeoutError:
                logger.warning(
                    f"⏰ [Simulator-{worker_id}] Global timeout or task wait exceeded."
                )
                self.stats["errors"] += 1
            except Exception as e:
                ...
                if cand is not None:
                    try:
                        self.stats["active_simulations"] = max(
                            0, int(self.stats.get("active_simulations", 0) or 0) - 1
                        )
                        self.pending_signatures.discard(
                            parameter_agnostic_signature(cand.expression)
                        )
    ...
    async def _handle_simulation_result(self, res, cand: AlphaCandidate):
        ...
        if not gate["passed"]:
            self.tracker.mark_rejected_by_id(
                row_id, reason=f"gate:{gate['stage']}:{gate['reason']}"
            )
            self.stats["rejected"] += 1
            self.stats["gate_rejections"] += 1
            self.allocator.update(arm, reward)
            logger.info(
                "[GATE] D%s rejected=%s | expr_id=%s",
                res.delay,
                gate["stage"],
                safe_expr_ref(res.expression),
            )
            return
        ...
        if passes_quality_gate_v2(res):
            ...
            logger.info(
                "[SUCCESS] D%s Sharpe=%.2f | expr_id=%s",
                res.delay,
                res.sharpe,
                safe_expr_ref(res.expression),
            )
            ...
            if self.qd_archive.maybe_update_archive(
                res.expression, quality=q_val, novelty=novelty, descriptor=descriptor
            ):
            ...
            try:
                ...
                if acceptance_rates:
                    ...
                    logger.debug(
                        ...
                        {
                            arm: f"{v['p_accept']:.2f}({v['resolved']})"
                            for arm, v in acceptance_rates.items()
                        },
                    )
            except Exception as e:
                logger.warning(
                    "⚠️ [Allocator] Failed to update acceptance priors: %s", e
                )
    ...
    async def status_monitor(self):
        ...
        while self.is_running:
            ...
            throughput = (
                (self.stats["simulated"] / (elapsed / 3600)) if elapsed > 0 else 0
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--score", type=float, default=PRE_RANK_THRESHOLD, help="Min pre-rank score"
    )
