...
# The text from the user's prompt (extracted for the script)
RAW_ALPHAS = [
...
def translate(formula):
    ...
    # Apply standard mappings (case-insensitive)
    ...
    # Using a more robust approach for nested ternary
    def replace_ternary(match):
        cond, true_val, false_val = match.groups()
        return f"if({cond.strip()}, {true_val.strip()}, {false_val.strip()})"

    # Handle power x^y -> power(x, y)
    # We'll use a simple loop to find matched parens or simple tokens
    def handle_power(f):
        # Find x^y where x is a simple token or balanced parens
        # This is a bit complex for regex, so we do a simple one and repeat
        changed = True
        while changed:
            changed = False
            # Match word^word or word^number or (expr)^number or (expr)^word
            match = re.search(r'(\([^\^()]+\)|[a-zA-Z0-9.]+)\^([a-zA-Z0-9.]+)', f)
            if match:
                base, exp = match.groups()
                # If balanced parens
                f = f.replace(match.group(0), f"power({base}, {exp})")
                changed = True
        return f

    # Handle ? : (Simplified for common patterns in 101)
    # Most are like (cond ? val1 : val2)
    def handle_ternary(f):
        changed = True
        while changed:
            changed = False
            # Find (A < B) ? C : D
            # We look for ? and then backtrace for if/balanced parens
            # In 101, it's usually (cond) ? true : false
            match = re.search(r'\(([^?()]+)\)\s*\?\s*([^?()]+)\s*:\s*([^?()]+)', f)
            if match:
                cond, t, e = match.groups()
                f = f.replace(match.group(0), f"if({cond.strip()}, {t.strip()}, {e.strip()})")
                changed = True
        return f

    formula = handle_ternary(formula)
    formula = handle_power(formula)
    
    # Final cleanup
    # Replace (with space) or double math ops if any
...
result = []
for i, f in enumerate(RAW_ALPHAS):
    translated = translate(f)
    result.append(translated)

with open('d:/alpha-factory-private/translated_101.txt', 'w') as out:
    out.write("ARXIV_101 = [\n")
    for r in result:
        out.write(f'    "{r}",\n')
    out.write("]\n")
