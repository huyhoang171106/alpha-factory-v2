...
def translate(formula):
    ...
    # But be careful not to lowercase price fields if they are already lowercase
    # Actually most things in WQ Brain are lowercase.
    
    # 1. Standardize spacing and basic operators
    formula = formula.replace('^', ', ') # Wait, x^y -> power(x, y) or signed_power(x, y)? Usually power.
    # We will handle ^ later with regex.
    
    # 2. Map operators
    mapping = {
        r'\bstddev\(': 'ts_std_dev(',
        r'\bcorrelation\(': 'ts_corr(',
        r'\bdelta\(': 'ts_delta(',
        r'\bdelay\(': 'ts_delay(',
        r'\bsum\(': 'ts_sum(',
        r'\bcovariance\(': 'ts_covariance(',
        r'\bproduct\(': 'ts_product(',
        r'\bts_rank\(': 'ts_rank(',
        r'\bTs_Rank\b': 'ts_rank',
        r'\bTs_ArgMax\b': 'ts_arg_max',
        r'\bTs_ArgMin\b': 'ts_arg_min',
        r'\bSignedPower\b': 'signed_power',
        r'\bdecay_linear\(': 'ts_decay_linear(',
        r'\bindneutralize\(': 'group_neutralize(',
        r'\bIndNeutralize\(': 'group_neutralize(',
        r'\bIndClass\.sector\b': 'sector',
        r'\bIndClass\.industry\b': 'industry',
        r'\bIndClass\.subindustry\b': 'subindustry',
        r'\bts_min\(': 'ts_min(',
        r'\bts_max\(': 'ts_max(',
        r'\bts_argmax\(': 'ts_arg_max(',
        r'\bts_argmin\(': 'ts_arg_min(',
    }
    
    for pattern, replacement in mapping.items():
        formula = re.sub(pattern, replacement, formula, flags=re.IGNORECASE)

    # 3. Handle ternary operator (condition) ? true_val : false_val
    # simple version: if(condition, true_val, false_val)
    # WQ Brain handles 'if'
    while '?' in formula:
        # This is tricky with nested parens. 
        # For simplicity, we can use a more robust regex or manual parsing if needed.
        # But let's try a regex for the common pattern: (cond) ? true : false
        match = re.search(r'\(([^?]+)\)\s*\?\s*([^:]+)\s*:\s*([^)]+)', formula)
        if match:
            cond, true_val, false_val = match.groups()
            formula = formula.replace(match.group(0), f"if({cond.strip()}, {true_val.strip()}, {false_val.strip()})")
        else:
            # Fallback for non-parenthesized cond or nested
            break
            
    # 4. Handle power operator x^y -> power(x, y)
    # This is also complex due to parens
    while '^' in formula:
        # Find x^y. x can be a number, a variable, or a parenthesized expression.
        # Let's use a simpler approach: replace ^ with power function if it's not handled.
        # Actually, let's just do a simple regex since most are (expr)^n
        match = re.search(r'(\([^)]+\)|[a-zA-Z0-9.]+)\^([a-zA-Z0-9.]+)', formula)
        if match:
            base, exp = match.groups()
            formula = formula.replace(match.group(0), f"power({base}, {exp})")
        else:
            break
...
for line in text.split('\n'):
    ...
    if match:
        if current_alpha:
            alphas.append(translate(" ".join(current_formula)))
        current_alpha = int(match.group(1))
        # Part of the formula might be on the same line
        formula_part = line[match.end():].strip()
        if formula_part:
...
# Output as a list of strings for python
print("ARXIV_101 = [")
